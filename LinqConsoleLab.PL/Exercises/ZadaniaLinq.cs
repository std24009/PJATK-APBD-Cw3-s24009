...
public sealed class ZadaniaLinq
{
    ...
    {
        var studentInfo = DaneUczelni.Studenci.Where(s => s.Miasto == "Warsaw")
    ...
    {
        var students = DaneUczelni.Studenci.Select(s => s.Email);
        return students;
    }
    ...
    {
        var studenci = DaneUczelni.Studenci.OrderBy(s => s.Nazwisko).ThenBy(s => s.Imie)
    ...
    {
        var subject = DaneUczelni.Przedmioty.Where(s => s.Kategoria == "Analytics")
            .Take(1)
    ...
    {
        var exist = DaneUczelni.Zapisy.Any(s => s.CzyAktywny == false);

        IEnumerable<string> result = new[] { exist ? "Tak" : "Nie" };
    ...
    {
        var ileJest = DaneUczelni.Zapisy.Count(s => s.CzyAktywny);
        IEnumerable<string> result = new[] { ileJest.ToString() };
        return result;
    ...
    {
        var uniqeCities = DaneUczelni.Studenci.DistinctBy(s => s.Miasto)
            .OrderByDescending(s => s.Miasto)
            .Select(s => s.Miasto);
        return uniqeCities;
    ...
    {
        var newJoin = DaneUczelni.Zapisy.OrderByDescending(s => s.DataZapisu)
    ...
    {
        var result = from z in DaneUczelni.Zapisy
    ...
    public IEnumerable<string> Zadanie14_SredniaOcenaNaPrzedmiot()
    {
        var result = from z in DaneUczelni.Zapisy
            ...
            group z by p.Nazwa
            into g
            select $"{g.Key}: {g.Average(z => z.OcenaKoncowa):F2}";
    ...
    {
        var result = DaneUczelni.Prowadzacy.GroupJoin(
            DaneUczelni.Przedmioty,
            pr => pr.Id,
            p => p.ProwadzacyId,
            (pr, grupa) => $"{pr.Imie}, {pr.Nazwisko}: {grupa.Count()}");
    ...
    {
        var result = from s in DaneUczelni.Studenci
            join z in DaneUczelni.Zapisy on s.Id equals z.StudentId
            where z.OcenaKoncowa != null
            group z by new { s.Imie, s.Nazwisko }
            into g
            select $"{g.Key.Imie} {g.Key.Nazwisko}: {g.Max(z => z.OcenaKoncowa)}";
